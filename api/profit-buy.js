/**
 * Vercel API — 매수 조건 잡 관리
 * GET    /api/profit-buy           → 잡 목록
 * POST   /api/profit-buy  body:job → 잡 등록 / 교체
 * DELETE /api/profit-buy?ticker=xx → 잡 취소
 */
export default async function handler(req, res) {
  ...
  const FILENAME = 'profit_buy_jobs.json';
  const ghHeaders = {
  ...
  if (req.method === 'POST') {
    ...
    const now = new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }).slice(0, 16);
